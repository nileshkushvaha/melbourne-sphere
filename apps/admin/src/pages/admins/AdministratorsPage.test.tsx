...
describe('Administrators page', () => {
  ...
  it('creates an administrator through the dialog and surfaces API conflicts', async () => {
    const ue = user();
    renderWithProviders(<AppRoutes />, { initialEntries: ['/admin/admins'] });
    await screen.findByText('New Person');
    await ue.click(screen.getByRole('button', { name: /new administrator/i }));
    const dialog = await screen.findByRole('dialog');
    await ue.type(within(dialog).getByLabelText(/email/i), 'taken@example.com');
    await ue.type(within(dialog).getByLabelText(/display name/i), 'Taken Person');
    await ue.click(within(dialog).getByRole('button', { name: /create and send setup link/i }));
    expect(await within(dialog).findByText(/already exists/i)).toBeInTheDocument();
    await ue.clear(within(dialog).getByLabelText(/email/i));
    await ue.type(within(dialog).getByLabelText(/email/i), 'fresh@example.com');
    await ue.click(within(dialog).getByRole('button', { name: /create and send setup link/i }));
    const post = calls.find((c) => c.method === 'POST' && c.url === '/api/v1/admin/admins' && c.body?.includes('fresh@example.com'));
    expect(post).toBeDefined();
    expect(JSON.parse(post!.body!)).toEqual({ email: 'fresh@example.com', displayName: 'Taken Person', roleKeys: ['super_admin'] });
  });
});
