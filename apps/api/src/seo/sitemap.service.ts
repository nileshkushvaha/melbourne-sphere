...
export const SITEMAP_SECTIONS = ['businesses', 'editorial', 'taxonomies'] as const;
...
export class SitemapService {
  ...
  private async taxonomies(): Promise<SitemapEntryDto[]> {
    ...
    const entries: SitemapEntryDto[] = [
      ...categories.filter((row) => hasText(row.description)).map((row) => ({ path: `/directory/category/${row.slug}`, lastModified: row.updatedAt.toISOString() })),
      ...areas.filter((row) => hasText(row.editorialIntro)).map((row) => ({ path: `/directory/area/${row.slug}`, lastModified: row.updatedAt.toISOString() })),
