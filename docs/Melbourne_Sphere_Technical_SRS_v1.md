...
| 1.8          | 12 Sep 2026 | The recorded approval is removed from testimonials entirely, at client instruction: a testimonial is entered by an administrator who holds the permission to enter one, and publication is decided by status alone. Requirement IDs affected: TSTM 002 (amended), TSTM 001 (model). Client/partner authorisation under PTNR 003 is unchanged. Data impact: **destructive** — `testimonials.approvedByAdminId`, `approvedAt` and `approvalNote` are dropped, with the foreign key to `admin_users`, in migration `20260912161741_remove_testimonial_approval`. Any approvals already recorded are destroyed and cannot be restored by reversing the migration: re-adding the columns would produce empty records, not the originals. The `website.testimonials.approve` permission and its endpoint are removed. Security impact: none — no authorisation is weakened; one permission ceases to exist because the action it guarded no longer exists. The residual risk is legal rather than technical and is restated in TSTM 002: the system no longer holds, and cannot reconstruct, evidence that a person consented to being quoted, so that evidence must be kept outside it. Test impact: showcase integration and admin screen tests updated; the admin screen now names the two states "active" and "inactive" in its own interface while the stored vocabulary is unchanged. Approver: client instruction of 12 Sep 2026, recorded by the delivery technical lead. | Approved by client instruction; pending countersignature at baseline acceptance |
...
| 1.9          | 14 Sep 2026 | Public navigation becomes administrator-managed, WordPress-style, at explicit client instruction: named menus built from pages, posts, blog categories and tags, business categories, local areas, published businesses, site routes, custom links and headings, ordered and nested by drag and drop, with icons, descriptions and a button style, and assigned to four fixed locations — primary (header, up to three levels with dropdowns and flyouts and a mobile drawer), secondary (the contact strip above the header), footer (each top-level item a column) and footer bottom (beside the copyright). Requirement IDs affected: UX 002 (amended: the listed items become the shipped default menus rather than a fixed list), MENU 001–006 (new, section 26), RBAC 013 (catalogue extended with `website.menus.view` and `website.menus.manage`), the section 14 entity table and the T17 verification group (amended). Reason: client requirement to change navigation without a release. Data impact: additive — migration `20260914120000_website_menus` creates `menus`, `menu_locations` (with its four fixed rows) and `menu_items`; linked records are referenced by typed foreign keys that clear, never cascade, when the record is deleted. Default menus reproducing the previous navigation are written by the idempotent `menus:seed` command, never by the migration, and never overwrite an assignment. **Behaviour change:** a newly published information page no longer joins the footer automatically; an administrator adds it from the Pages panel, and the policy links still appear only while their pages are published. Security impact: custom destinations validated at write time on the ALRT 005 rules, extended to a single `mailto:` address and a `tel:` number; publication filtering happens in the API; the link-source read exposes titles, addresses and state only and never an unpublished listing; viewing and managing are separate permissions. Test impact: domain tree and link rules, resolver, admin tree operations, web navigation and fallback tests, a menus integration suite against the real database, and Playwright drawer and submenu journeys. Approver: client instruction of 14 Sep 2026, recorded by the delivery technical lead. | Approved by client instruction; pending countersignature at baseline acceptance |
| 1.10         | 14 Sep 2026 | Article publishing made workable for a non-technical editor, at explicit client instruction (first part of the articles programme). Requirement IDs affected: BLOG 001, BLOG 002, BLOG 003, BLOG 004, BLOG 005, COM 001 and COM 002 (amended); BLOG 006 (new). BLOG 006 (blog analytics events): only with the visitor's analytics consent, the blog counts shares and link copies, reading an article to 75% (once per view), clicks on related articles, business cards and table-of-contents entries, video and map loads, and blog searches by result count only; the words searched for, form contents and personal data are never sent, events go only to the configured Google analytics tag, and nothing is recorded on private previews. BLOG 005 (author pages, client instruction of 15 Sep 2026): the MVP exclusion of author archive pages is lifted for authors with published work — an active author with at least one published article has a public page showing their published profile and their articles (12 per page, newest first), listed in the sitemap and linked from bylines; an author without published articles, or an inactive one, has no page, so no empty archive exists. COM 001–002 (comment replies, client instruction of 15 Sep 2026): readers may reply to approved comments; threads have two levels, and a reply to a reply joins the same thread. Replies follow the same fields, acknowledgement, limits and moderation as comments, and a reply is shown only while the comment it answers is approved. Moderators may reply as the Melbourne Sphere team; such replies are published immediately, marked as the team's, and record the writing administrator privately. Pagination is of top-level comments, 20 per page, each with its approved replies, oldest first. No notification emails are sent (migration `20260915090000_comment_replies`). BLOG 005: numbered blog, category and tag pages are crawlable, each later page canonical to itself and page 1 to the plain address, and a page beyond the last is not found; the blog publishes an RSS 2.0 feed of the newest 20 published articles with their summaries, advertised on every page. Readers can search published articles by keyword (every word must match, most relevant first); search results are not indexed, and what was searched for is not recorded. A publisher can feature up to three published articles, which lead the home page's blog section and page one of the blog; an article stops being featured when it is unpublished or archived. The editor offers advice on the focus phrase, length, headings, image descriptions, internal links and sentence length; the advice never prevents publishing. BLOG 004: article text may contain captioned images at normal or full width (each with a required description), tables, links chosen from the site's own content, YouTube videos and Google Maps embeds, and cards for published businesses. Videos, maps and cards are stored as validated inert markers, never frames; anything else is removed by the SEC 001 allowlist. The public page loads nothing from YouTube or Google until the reader chooses to (or has chosen to always load them in that browser), and a card whose business is no longer published is not shown. An article with three or more sections shows a table of contents linking to them, derived from its headings when the page is shown rather than stored. BLOG 003: unsaved content can be previewed in the editor, rendered by the API without being stored, and the saved draft can be opened in the public design through a random ten-minute token bound to the editor's session (no-store, noindex, no-referrer, disallowed in robots). Draft saves that change the title or text now keep a revision with its editable source (latest 50 per article); versions can be compared and restored, the current text being kept first; unsaved work is autosaved privately per administrator (`post_autosaves`, migration `20260914180000_post_autosave_and_revision_source`) without changing the article or its version. (0) Each administrator may record a default author profile that new articles pre-select (`admin_users.defaultAuthorId`, migration `20260914170000_admin_default_author`, set to null if the profile is deleted); it is a private preference of the signed-in administrator, recorded in the activity log and never part of any public output, so public author identity stays separate from login identity. (1) The admin evaluates the BLOG 002 requirements live while the article is written, using the same shared rule set and wording the API enforces, and each unmet requirement links to its field. (2) Publishing or scheduling saves outstanding edits first, so what goes live is what is on screen; a brand-new article can be published directly. (3) A summary left empty is written from the article's opening text (ending at a sentence or word boundary) and the editor is told so; it is never a silent substitution. (4) Scheduled publication runs only in the worker's `content.publish-scheduled` task, now every minute; the API's own timer is removed so the two can no longer record publication differently. At the scheduled time the requirements are checked again; a qualifying article is published under the same version guard, audit entry and cache invalidation as a manual publication, and one that no longer qualifies (for example its author was deactivated) returns to draft with the reason stored and shown in the editor, the article list and the dashboard. Data impact: additive — migration `20260914160000_post_publish_failure` adds `posts.publishFailure`. Related defect fix recorded without a requirement change: images inside article, page, author-bio, FAQ and blog landing text now count as in use under MED 004/DAT 003 (migration `20260914150000_content_media_references`, backfill command `media:backfill-content`; no image had been lost on the development database). Security impact: none; publication permissions unchanged. Test impact: embed parsing, sanitiser marker and public body-splitting tests, shared rule and summary unit tests, worker publication task tests (publish, refuse to draft, concurrent change), blog and media integration expectations updated, admin list wording. Approver: client instruction of 14 Sep 2026, recorded by the delivery technical lead. | Approved by client instruction; pending countersignature at baseline acceptance |
| 1.11         | 15 Sep 2026 | A map of Melbourne above the footer replaces the home page testimonials, at explicit client instruction. Requirement IDs affected: BUS 003 (amended), TSTM 004 (amended), CFG 001 (amended). BUS 003: every public page (not the admin, not private previews) shows a full-width Google map of Melbourne above the footer, displayed directly and lazy-loaded, so the visitor's browser requests it from Google (which may set cookies) when the map scrolls into view, without a separate choice (client decision of 15 Sep 2026; the privacy notice must say so); business pages keep their directions link, and no business location is tracked. Article maps and videos remain click-to-load. TSTM 004: testimonials are no longer shown on the public site; their administration and stored records are retained unchanged. CFG 001: General settings gain an optional map (Google Maps "Embed a map" HTML or address, validated by the same rules as article maps); empty shows the built-in Melbourne map. Data impact: none — the setting lives in the existing settings document. Security and privacy impact: only Google Maps embed addresses are accepted, re-checked by the public site; the map is a third-party request on every public page view that scrolls to it. Test impact: settings validation and domain map-source tests. Approver: client instruction of 15 Sep 2026, recorded by the delivery technical lead. | Approved by client instruction; pending countersignature at baseline acceptance |
| 1.12         | 15 Sep 2026 | Published prices and paid guest posts, at explicit client instruction. Requirement IDs affected: FUT 002, CFG 001, BLOG 001, BLOG 004, ENQ 002 (amended). FUT 002: the site publishes two prices — Guest Post, $39 one time, and Business Listing, $69 per year, Australian dollars including GST — on the home and contact pages; orders are taken through the contact form, whose topic the chosen plan preselects; the system still processes no payments and holds no invoices, subscriptions or card data, and featured placements are still not sold. CFG 001: General settings hold the two plans (name, price, billing period, summary, up to six included points) and whether prices are shown; the shipped defaults are the prices above. BLOG 001/004: an article may be marked as a paid guest post; the public site labels it "Guest post" on its card and article, states before the text that it is a paid guest post reviewed by the editors, and marks its outbound links rel="sponsored"; the wording that articles are written by the editors, not by businesses, is replaced accordingly. ENQ 002: the contact topics add Business listing plan and Guest post. Data impact: additive — migration `20260915120000_post_guest_post` adds `posts.guestPost` (default false); prices live in the existing settings document. Content impact: the published Terms of Use still say businesses are not charged and must be edited by the client; the seeded text for new installations is updated. Security impact: none; no payment data is collected. Test impact: pricing rules, settings validation and sponsored-link tests. Approver: client instruction of 15 Sep 2026, recorded by the delivery technical lead. | Approved by client instruction; pending countersignature at baseline acceptance |
...
**UX 002 —** Public navigation shall include Home, Businesses, Blog, About and Contact, plus an email based "Add or update a business" action. Footer links shall include Privacy, Terms and Review guidelines. Navigation, breadcrumbs and content headings shall remain usable by keyboard and screen reader. *Amended in 1.9 by client instruction:* these items are the default menus shipped by the menus seed, and administrators manage the navigation thereafter under MENU 001–006.
...
**BLOG 001 —** Administrators alone create and manage articles. Each article has title, unique stable slug, excerpt, sanitized rich content, author attribution, primary category, optional tags, cover image, publication date, updated date and SEO fields. Public author identity is separate from admin login identity and email. *Amended in 1.10 by client instruction:* an administrator may choose a default author profile that new articles pre-select; the link is a private preference and never public.

**BLOG 002 —** States are draft, scheduled, published and archived. Require title, content, author, active category and valid slug before publish/schedule. Store scheduled timestamps in UTC while the admin selects and sees Australia/Melbourne time with offset. Scheduled publishing runs idempotently with a periodic catch up scan; a missed job shall not leave due posts stranded. *Amended in 1.10 by client instruction:* the admin shows these requirements live as the article is written; publishing or scheduling saves outstanding edits first; an empty summary is written from the opening text and shown to the editor; scheduled publication runs in the worker and re-checks the requirements at the scheduled time, returning an article that no longer qualifies to draft with the reason recorded.

**BLOG 003 —** Draft preview requires an authenticated authorized admin and a short lived preview route, sets noindex and private no-store and excludes draft URLs from sitemaps. A public unauthenticated draft lookup returns 404. Updates to a published article create a revision and invalidate public content; preserve first publication date separately from updatedAt. *Amended in 1.10 by client instruction:* the editor may preview unsaved content rendered by the API without storing it; the preview route is a random token valid ten minutes, bound to the issuing administrator's session, served no-store, noindex and no-referrer, and excluded from robots and sitemaps. Revisions are also kept for draft saves that change the title or text, with the editable source, capped at the latest 50 per article, and can be compared and restored (restore keeps the current text as a revision and is audited); unsaved work is kept automatically as a private per-administrator copy that never changes the article, and browser copies are cleared on sign-out.
...
| Menu | name, audit fields, version | Unique name; at most 20 menus; deleting an assigned menu is refused (1.9 MENU 001) |
| MenuLocation | location key (primary, secondary, footer, footer bottom), assigned menu, version | One row per fixed location; assigned menu restricted on delete (1.9 MENU 003) |
| MenuItem | parent, position, kind, typed reference to the linked record, route key or validated URL, label, title attribute, description, icon, style, new tab, nofollow | At most three levels and 60 items; references clear on delete; destinations validated on save (1.9 MENU 001, MENU 004) |
...
**TSTM 001 — Model.** A testimonial holds the person's display name, their role or relationship to the business, the quoted text, an optional associated directory listing, an optional approved image, a display order, a publication status, audit fields and an optimistic concurrency version. The quote has a documented maximum length and is sanitised. *(The consent and approval fields were removed in 1.8 by client instruction; see TSTM 002.)*

**TSTM 002 — Authorship and publication.** ~~A testimonial shall not be published without a recorded approval~~ *(publication gate withdrawn in 1.5)*. ~~A testimonial may record an approval that names the approving administrator, the time, and how consent was obtained~~ *(the approval record itself withdrawn in 1.8 by client instruction)*. A testimonial is created and edited only by an administrator holding the testimonial permissions, and whether it appears on the site is decided by its publication status alone. Attribution shall be honest; the system shall not fabricate testimonials, and placeholder or sample testimonials shall never exist in production, consistent with CFG 002. The residual risk is accepted by the client and recorded in the change log: the system holds no evidence that the person agreed to be quoted, and none can be reconstructed, so that evidence must be kept outside this system.
...
Added at revision 1.9 by client instruction.

**MENU 001 — Model.** A menu holds a unique name, audit fields and an optimistic concurrency version. Four locations are fixed in code — primary, secondary, footer and footer bottom — and each shows at most one menu; one menu may serve several locations. A menu item holds its parent (items nest at most three levels), its position among its siblings, its kind — custom link, heading, site route, information page, post, blog category, blog tag, business category, local area or business — a typed reference to the linked record where the kind has one, an optional label that otherwise falls back to the linked record's title, an optional title attribute and description, an optional icon from a shared code-declared library, a link or button style, and whether it opens in a new tab and carries `nofollow`. Deleting a linked record clears the reference and never deletes the item. A site holds at most 20 menus and a menu at most 60 items; every text field has a documented maximum length.

**MENU 002 — Administration.** Administrators holding the menu permissions manage menus on one screen: content panels list pages, posts, blog categories and tags, business categories, local areas, published businesses and site routes with most recent, view all and search, from which checked items are added; custom links and headings are added directly. Items are ordered and nested by drag and drop, and every drag has a keyboard-operable equivalent (WCAG 2.5.7). A menu is saved as a whole tree in one transaction carrying `expectedVersion`, writes an activity event, and invalidates the public shell when the menu is shown anywhere. The content read used by the panels returns titles, addresses and publication state only, never an unpublished listing, and requires only the menu permission. A menu cannot be deleted while it is assigned to a location.

**MENU 003 — Locations and depth.** Primary: up to three levels, eight top-level items and two button-style items, rendered as dropdowns and flyouts in the header and as a drawer below the desktop breakpoint. Secondary: one level, six items, on the right of the contact strip. Footer: two levels, where each top-level item is a column heading (at most four) with up to twelve links. Footer bottom: one level, six items, beside the copyright notice. A save is validated against every location the menu is assigned to, and an assignment is refused when the menu does not fit the location. The primary location cannot be cleared. Default menus reproducing UX 002 are created by an idempotent seed that never replaces an existing assignment.

**MENU 004 — Link safety.** A custom destination is validated server side at write time on the rules of ALRT 005: a site-relative path, an in-page fragment or an http(s) URL without credentials, plus a single `mailto:` address without additional headers and a `tel:` number. `javascript:`, `data:`, protocol-relative and otherwise unsafe destinations are refused on save. A link that opens a new tab carries `noopener noreferrer` and says so to assistive technology; `nofollow` is added only where the administrator chose it.

**MENU 005 — Public behaviour.** The API resolves every item's address and filters publication: an item whose target is unpublished, scheduled, inactive or deleted is hidden together with everything nested under it, a heading left without visible children is not rendered, and the FAQ route appears only while a question is published. Navigation is server rendered. A menu change invalidates the shell cache, and the web tier also refreshes menus when any linked content is purged. When menus cannot be read, the shell falls back to the product routes of UX 002 only, never to a link that might answer 404. A newly published information page is not added to any menu automatically.

**MENU 006 — Accessibility.** Menus are disclosure navigation, not ARIA menus: a parent with an address remains a link and its submenu opens from a separate button exposing `aria-expanded`; Escape closes the innermost open submenu and returns focus to its button; moving focus away closes submenus; hover opens submenus only on hover-capable pointers, and every link remains reachable without JavaScript. The compact navigation is a modal dialog drawer that makes the page behind it inert, closes on Escape and returns focus to its trigger. The current page carries `aria-current`, icons are decorative, targets are at least 44 pixels, and layouts hold at 320 pixels.

Acceptance for section 26: unpublished and out-of-window content proven absent from every public page, public API response and sitemap; sanitisation proven on every rich field; publication and removal proven to invalidate the relevant caches within the CACHE 001 bound, and an alert removal proven urgent under CACHE 002; alert selection proven deterministic and proven correct across a daylight saving transition; alert dismissal proven keyboard operable, proven version-aware and proven safe when client storage is unavailable; contrast and live-region semantics proven per severity; FAQ disclosure behaviour proven keyboard operable with correct heading structure; structured data proven to describe only visible content and proven gated; testimonial publication proven impossible without a recorded approval; partner logo publication proven impossible without recorded authorisation and alternative text; empty states proven to omit their sections cleanly; navigation menus proven to hide unpublished, inactive and deleted targets with their children, proven to refuse unsafe destinations, broken or over-deep trees and stale saves without writing, proven to refuse deleting an assigned menu and clearing the primary location, and proven keyboard operable in both the desktop submenus and the mobile drawer; and every list, mutation and publication action proven permission-gated at the API independently of the interface.
