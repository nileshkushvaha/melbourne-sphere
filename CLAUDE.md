...
5. **Do not run test suites while iterating.** Write the code, then run the checks ONCE, at the end of the work: `pnpm lint`, `pnpm typecheck`, `pnpm contracts:check`, then `pnpm check` / `pnpm test:integration`. A focused single-file run counts as running tests. While implementing, verify by reading the code and with `tsc --noEmit`, which is cheap. The suites here are slow (API integration ~2 min, `pnpm test` several more), so repeated runs burn the user's waiting time and read as churn. If something fails at the end, fix all of it, then re-run once.
